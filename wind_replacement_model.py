...
condition = (
    ...
    (df['Wind MW(generation capacity)'] != 0)
)
...
cost_input = df[["Coal Plant/Lignite power plants", "phase", "Total Wind cost of allocating demand node j to facility i in period t (in €)"]]
...
cf_wind = 0.30
# hours_per_year = 8760
wind_df["value"] = wind_df["value"] * cf_wind
...
d_df = demand_wind_df.copy()               # has columns: ["j","t","value"]
a_df = d_df.copy()
