...
df["hybrid_cost"] = (
    df["solar_energy"] * df["Total Solar cost of allocating demand node j to facility i in period t (in €)"] + df["wind_energy"] * df["Total Wind cost of allocating demand node j to facility i in period t (in €)"]
) / df["total_energy"].replace(0, np.nan)  # Avoid div-by-zero

df["hybrid_cost"] = df["hybrid_cost"].fillna(0)


cost_it = df[["Coal Plant/Lignite power plants", "t", "hybrid_cost"]].rename(
    columns={"Coal Plant/Lignite power plants":"i", "hybrid_cost":"value"}
)
cost_it["i"] = cost_it["i"].astype(str)
...
#sets
j = Set(container=m, name="j", description="demand locations", records=j_labels)
i = Set(container=m, name="i", description="candidate locations", records=i_labels)
t = Set(container=m, name="t", description="periods", records=t_labels)

#parameters
#print(assignment_cost.to_string())

c = Parameter(m, name="c", domain=[i, t])
cost_it = (df.groupby(["Coal Plant/Lignite power plants","t"], as_index=False)["hybrid_cost"]
             .mean()
             .rename(columns={"Coal Plant/Lignite power plants":"i","hybrid_cost":"value"}))
c.setRecords(cost_it)
...
hybrid_df = (df.groupby(["Coal Plant/Lignite power plants","t"], as_index=False)["total_energy"]
               .sum()
               .rename(columns={"Coal Plant/Lignite power plants":"i","total_energy":"value"}))
s.setRecords(hybrid_df)
...
demand_df = (df.groupby(["Coal Plant/Lignite power plants","t"], as_index=False)["Coal MW replaced( demand)"]
               .sum()
               .rename(columns={"Coal Plant/Lignite power plants":"j","Coal MW replaced( demand)":"value"}))
d.setRecords(demand_df)
...
obj = Sum((i, j, t), c[i, t] * x[i, j, t])  # objective function
